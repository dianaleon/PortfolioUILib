...
import com.portfolio.components.PhotoTextGridListItemSocios;
...
public class CatalogoActivity2 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String code = catalogoPage.getType().getType();
		
		/*********** si la pagina es testimonios o servicios (4 bloques por pagina) ******************/
		if(code.equalsIgnoreCase("catalogo"))
		{
					...
					switch (object.getType()) {
						case IPageObject.type_text:
							...
							item.fill(textObject.getContent_img(), textObject.getTitle(),
									textObject.getContent(),textObject.getTextColor() ,textObject.getTextColor(),
						...
						case IPageObject.type_image:
							...
							itemImage.fill(imageObject.getContent_img(), imageObject.getTitle(),
									imageObject.getContent(),imageObject.getTextColor(),imageObject.getTextColor(),
		...
		/************ si la pagina es socios (2 bloques por pagina)  *****************/
		}
		else if (code.equalsIgnoreCase("text"))
		{
			for (int index = 0; index < objetos.size(); index++) {
				IPageObject object = objetos.get(index);
				switch (object.getType()) {
					case IPageObject.type_text:
						ITextObject textObject = (ITextObject) object;
						PhotoTextGridListItemSocios item = new PhotoTextGridListItemSocios(this);
						item.fill(textObject.getContent_img(), textObject.getTitle(),
								textObject.getContent(),textObject.getTextColor() ,textObject.getTextColor(),
								textObject.getStartColorBackground(),
								textObject.getEndColorBackground(),
					...
					case IPageObject.type_image:
						IImageObject imageObject = (IImageObject) object;
						PhotoTextGridListItemSocios itemImage = new PhotoTextGridListItemSocios(this);
						itemImage.fill(imageObject.getContent_img(), imageObject.getTitle(),
								imageObject.getContent(),imageObject.getTextColor(),imageObject.getTextColor(),
								imageObject.getStartColorBackground(),
								imageObject.getEndColorBackground(),
